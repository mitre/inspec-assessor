...
We will run the profile now using the default inputs.
`inspec exec https://github.com/CMSgov/redhat-enterprise-linux-8-stig-baseline -t docker://nginx --reporter progress-bar json:before_inputs.json`
...
Now we will run the profile again using our new updated input 
`inspec exec https://github.com/CMSgov/redhat-enterprise-linux-8-stig-baseline -t docker://nginx --reporter progress-bar json:after_inputs.json`

### 8.3. Exercise 1 NGINX

Now that we know the process of how to update our inputs file for the redhat 8 profile, this time lets try the same thing again but with the [nginx](https://github.com/mitre/nginx-baseline) profile.

Steps
1) Go to the [nginx profile page](https://github.com/mitre/nginx-baseline)
...
Now lets choose a different profile from [SAF site](https://saf.mitre.org/#/validate). After you choose your profile that you want to run, go over to [Docker Hub](https://hub.docker.com/) and search for a container for your intended profile. Once you do, you can run the `docker pull` command listed on the page to pull that container into your environment and you can start executing profiles using the steps of the above exercise.